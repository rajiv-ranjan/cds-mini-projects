...
def ice_break_with(name: str,llm) -> str:
    ...
    twitter_username = twitter_lookup_agent(name=name,agent_llm=llm)
    tweet_collection = scrape_twitter_profile(username=twitter_username, mock=True)
    ...
    summary_prompt_template = PromptTemplate(
        input_variables=["linkedin_information","twitter_posts"], template=summary_template
    )
    ...
    try:
        res = summary_chain.invoke(input={"linkedin_information": linkedin_data, "twitter_posts": tweet_collection})
        print(res)
